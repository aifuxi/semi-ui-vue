...
当前已完成 Button（Button、ButtonGroup、SplitButtonGroup）、Divider、Icon、Space、FloatButton（含 FloatButtonGroup）与 Layout（含 Header、Content、Footer、Sider）垂直切片。Icon 同时覆盖稳定版 523 个图标、Lab 84 个图标与生成漂移检查；每个已完成切片均包含 Vue API、中英文文档、单元/SSR/Chromium 对照、逐组件样式和真实 tarball 验证。其余组件仍按上游 Inventory 分阶段推进。唯一参考源码位于只读 submodule `vendor/semi-design`。

...
- **文档：**已统一为 Nuxt（Rspack builder）。覆盖账本记录 859/859 个上游中文 Demo 已映射至双语实现；映射不等于视觉与行为验收通过。有效验收数量见[覆盖账本](docs/documentation/coverage.json)，下一批及剩余工作见[双线计划](docs/documentation/batch-plan.md)，固定 React 对照的样式、几何、像素与交互门槛见[文档流程](docs/documentation/workflow.md)。

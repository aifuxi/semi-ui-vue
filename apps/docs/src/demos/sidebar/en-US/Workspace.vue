...
const options = [
  { icon: h(IconSearch), name: 'Search result', key: 'searchResult' },
  { icon: h(IconBriefStroked), name: 'File preview', key: 'filePreview' },
  { icon: h(IconCodeStroked), name: 'Code preview', key: 'codePreview' },
  { icon: h(IconModalStroked), name: 'Browser', key: 'network' },
];
function expand(
  _event: MouseEvent,
  ...
  <ConfigProvider :locale="locale"
    ...
    >
      <div style="flex: 1; padding: 20px">
        Workspace placeholder<output v-if="copied">Copied</output>
      </div>
      <Sidebar
        :show-close="false"
        visible
        :motion="false"
        title="Agent's workspace"
        :mode="mode"

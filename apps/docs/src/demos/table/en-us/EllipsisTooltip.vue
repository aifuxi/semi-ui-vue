...
const data = makeData(3).map((record) => ({
  ...record,
  name: `${record.name} A very long document name ${record.name}`,
  owner: `${record.owner} ${record.owner} ${record.owner}`,
}));
...
columns.push({ dataIndex: 'operate', fixed: 'right', width: 100, render: () => h(IconMore) });
...
<template>
  <div>
    <Table :columns="columns" :data-source="data" :pagination="false" :scroll="{ x: 1200 }"></Table>
  </div>
</template>

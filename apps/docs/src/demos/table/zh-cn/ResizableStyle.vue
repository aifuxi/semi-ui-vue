...
const resizable = {
  onResizeStart: (column: TableColumnProps) => ({ ...column, className: 'table-demo-resizing' }),
  onResizeStop: (column: TableColumnProps) => ({ ...column, className: '' }),
};
</script>
...
<style scoped>
:deep(.table-demo-resizing) {
  border-right: 2px solid red;
}
:deep(.react-resizable-handle:hover) {
  background-color: red;
}
:deep(.table-demo-resizing:hover .react-resizable-handle) {

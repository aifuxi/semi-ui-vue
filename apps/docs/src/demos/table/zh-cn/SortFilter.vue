...
const columns: TableColumnProps[] = [
  ...queryColumns.slice(0, 2),
  statusColumn,
  ...queryColumns.slice(2),
];
columns[0] = { ...columns[0], sorter: (a, b) => String(a.name).length - String(b.name).length };
</script>

<template>
  <div>
    <Table :columns="columns" :data-source="data" :row-selection="{}" :scroll="{ y: 300 }"></Table>
  </div>
</template>

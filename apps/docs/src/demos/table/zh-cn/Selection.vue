...
const rowSelection = computed(() => ({
  ...
  getCheckboxProps: (record: Row) => ({ disabled: record.key === '2', name: String(record.name) }),
  onSelect: (record?: Row, checked?: boolean) => {
    action.value = `${record?.name}: ${checked}`;
  },
  onSelectAll: (checked?: boolean) => {
    action.value = `all: ${checked}`;
  },
...
<template>
  <div>
    ...
    ></Table>
    <p aria-live="polite">{{ selected.join(', ') }} {{ action }}</p>
  </div>
</template>

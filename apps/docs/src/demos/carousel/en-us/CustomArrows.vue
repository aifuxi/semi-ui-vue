...
import { Title, Paragraph } from '@aifuxi/semi-ui-vue/typography';
import { Space } from '@aifuxi/semi-ui-vue/space';
import IconArrowLeft from '@aifuxi/semi-icons-vue/icons/IconArrowLeft';
import IconArrowRight from '@aifuxi/semi-icons-vue/icons/IconArrowRight';
...
const images = ['/demos/one.svg', '/demos/two.svg', '/demos/photo.svg'];
</script>
<template>
  <div>
    <Carousel style="width: 100%; height: 400px" theme="dark" :auto-play="false"
      ><template #leftArrow><IconArrowLeft size="large" /></template
      ><template #rightArrow><IconArrowRight size="large" /></template>
      <div
        v-for="(text, index) in textList"
        :key="index"
        :style="{ backgroundSize: 'cover', backgroundImage: `url('${images[index]}')` }"
      >
        <Space
          vertical
          align="start"
          ...
          ><span
            ...
            aria-hidden="true"
          /><Title :heading="2" style="color: #1c1f23">{{ text[0] }}</Title
          ><Space vertical align="start"
            ><Paragraph style="color: #1c1f23">{{ text[1] }}</Paragraph
            ><Paragraph style="color: #1c1f23">{{ text[2] }}</Paragraph></Space
          ></Space
        >
      </div></Carousel
    >

...
<template>
  <Form style="max-width: 400px">
    <Form.InputGroup :label="{ text: 'Phone number', required: true }" label-position="top">
      <Form.Select
        field="phonePrefix"
        init-value="+86"
        :rules="[{ required: true }]"
        :option-list="['+1', '+852', '+86', '+81'].map((value) => ({ value, label: value }))"
        style="width: 150px"
        show-clear
      />
      <Form.Input
        field="phoneNumber"
        init-value="18912345678"
        :rules="[{ required: true }]"
        style="width: 250px"
        show-clear
      />
    </Form.InputGroup>
    <Form.Input field="name" label="Username" init-value="Semi" />

...
description: '通过 CSS 分层和公开 Token 配合使用 Tailwind CSS。'
slug: 'tailwind'
locale: 'zh-CN'
category: 'ecosystem'
order: 13
...
这些问题在其他组件库与 Tailwind 共同使用时候也会经常遇到，但 可以通过 CSS Cascade Layers 解决，建议按照本文说明，正确配置项目。
...
使用 Tailwind 时，Tailwind 通过 class 对 dom 进行样式修改，同样作用于 Semi 组件库。同时 Tailwind 默认开启了 [Preflight](https://tailwindcss.com/docs/preflight) 来重置浏览器默认样式。
...
将默认主题的编译后 CSS 放入 `semi` 层，使其优先于 Preflight，低于业务工具类。只从该分层入口加载主题，避免再次以未分层方式导入同一份样式。Vue 与 Nuxt 应用不需要上游 React 专用的 webpack/rspack 插件。
...
通过 `@import ... layer(semi)`，编译后的主题样式位于 `semi` 层。另外，我们也手动设置了项目中的 Tailwind 的各种类型的样式的 Layer。
...
在 Vue 的 `main.ts` 中导入上述 `app.css`；Nuxt 使用 `css: ['~/assets/app.css']`。按需组件样式也可以通过 `@import` 的 `layer(semi)` 形式载入。不要同时开启本文分层方案和另一套重复导入主题的插件。

Preflight 的影响与选择性导入方式见 [Tailwind 官方说明](https://tailwindcss.com/docs/preflight)。

...
List 不增加键盘选择状态。使用 Checkbox/Radio 提供选择语义；可点击行应包含原生按钮或链接。键盘示例与固定上游一致，在 window 上监听上下方向键并循环高亮；接入业务页面时请按需限定监听范围。
...
筛选示例响应完整输入值，适用于中文输入法与英文输入。多选初值与固定上游一致，按书名展开为字符数组，因此初始没有勾选项。新增操作按固定上游取当前长度对应的下一项，删除后再新增会与上游一致地出现重复书名。键盘示例沿用固定上游在 window 上监听上下方向键，不额外提供可聚焦容器。

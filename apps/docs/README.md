...
T1、T2、T3、T4 的 76 个组件、733 个可运行 Vue SFC，其余示例继续显示迁移状态。
...
- 不接 REPL；`DemoBlock` 直接运行已登记的 Vue SFC，并用同一文件的原始源码生成代码区。示例清单契约见
  `.vitepress/theme/demo/types.ts`；T1、T2、T3、T4 示例按稳定 ID 自动登记，未登记示例保持占位状态。
...
- Button、Input、Select、Form、Table、Modal、Tooltip、Upload、Tabs、Pagination、Navigation、Breadcrumb、
  ...
  Cropper、ScrollList、Feedback、ColorPicker、Slider、SideSheet、Collapse、Carousel、VideoPlayer、UserGuide、Popconfirm、Card、Rating、Calendar、List、JsonViewer、InputNumber、Typography、Resizable、AIChatDialogue、TagInput、Transfer、AutoComplete、Chat、Image、AIChatInput、Sidebar
  共 82 个页面的 API 已按公开 Vue 类型校准；

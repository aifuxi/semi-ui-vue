...
浏览器全局对象通过 getter 延迟传入，只在 `onMounted` 中求值。此例按挂载时的目标注册一次，并保存该目标用于卸载清理；卸载时不重新求值 getter，也不隐含支持挂载后的目标切换。
...
export function useEventListener(target, event, callback) {
  let mountedTarget;
  onMounted(() => {
    mountedTarget = toValue(target);
    mountedTarget?.addEventListener(event, callback);
  });
  onUnmounted(() => {
    mountedTarget?.removeEventListener(event, callback);

...
- Declare reactive state correctly
  - Use `ref()` as a normal default; choose shallow APIs by object identity and update semantics
...
### `ref()` is a normal default for primitive state

`ref(0)` is valid and idiomatic. Primitive values have no nested object graph to proxy, so do not replace primitive refs with `shallowRef()` on an assumed performance benefit. Use shallow APIs when object identity or root-only updates require them.

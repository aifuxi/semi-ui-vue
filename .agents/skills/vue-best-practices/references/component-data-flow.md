...
Vue 3.4+ 的简单双向绑定可用 `defineModel`；显式 `modelValue` prop + `update:modelValue` emit 在新版本中也有效。组件库需要区分 prop 缺省与显式传入、受控/非受控状态、默认值或多个事件的顺序时，按公开契约选择显式 props/emits，不要只为使用宏而改写既有状态语义。
...
**GOOD (explicit contract; also valid in Vue 3.4+):**

...
defineOptions({ name: 'SwitchSpin' });

const props = defineProps<{ size: SwitchSize }>();
const gradientId = `linearGradient-semi-switch-spin-${useId()}`;
const spinSize = props.size === 'default' ? 'middle' : props.size;
</script>

<template>
  <div :class="['semi-switch-loading-spin', 'semi-spin', `semi-spin-${spinSize}`]">

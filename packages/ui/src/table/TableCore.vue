...
import { useTableVirtualization, type VirtualTableRow } from './useTableVirtualization';
import {
  buildHeaderRows,
...
defineOptions({ name: 'TableCore', inheritAttrs: false });
...
const measuredTableWidth = shallowRef<number>();
const verticalScrollbarWidth = shallowRef<number>();
...
let previousVirtualNativeTop: number | undefined;
...
const expandedInnerWidth = computed(() =>
  anyFixed.value && measuredTableWidth.value !== undefined
    ? measuredTableWidth.value -
      tableNumbers.DEFAULT_CELL_PADDING_LEFT! -
      tableNumbers.DEFAULT_CELL_PADDING_RIGHT! -
      tableNumbers.DEFAULT_CELL_BORDER_WIDTH_LEFT! -
      tableNumbers.DEFAULT_CELL_BORDER_WIDTH_RIGHT! -
...
function virtualItemHeight(index: number, row: VirtualTableRow): number {
  const virtual = props.virtualized;
  const itemSize = typeof virtual === 'object' ? virtual.itemSize : undefined;
  if (typeof itemSize === 'function')
    return itemSize(index, {
      expandedRow: Boolean(row.expandedRow),
      sectionRow: Boolean(row.sectionRow),
    });
...
const virtualRows = computed<VirtualTableRow[]>(() => {
  ...
  for (const row of flatPageRows.value) {
    rows.push({ ...row, index: rows.length });
    if (
      !row.sectionRow &&
      expandedKeys.value.has(row.key) &&
      (props.expandedRowRender || slots.expandedRow)
    ) {
      rows.push({
        ...row,
        key: `${row.key}-expanded-row`,
        sourceKey: row.key,
        index: rows.length,
        expandedRow: true,
      });
...
const virtualViewport = computed(() =>
  typeof props.scroll?.y === 'number'
    ? props.scroll.y
    : tableNumbers.DEFAULT_VIRTUALIZED_BODY_HEIGHT,
);
const virtualColumnWidths = computed(() =>
  flatColumns.value.map(
    (column) =>
      measuredHeaderWidths.value.get(column.key) ??
      (typeof (column.__width ?? column.width) === 'number'
        ? Number(column.__width ?? column.width)
        : 0),
  ),
);
const virtualWidth = computed(() =>
  virtualColumnWidths.value.reduce((sum, width) => sum + width, 0),
);
const virtualList = useTableVirtualization({
  rows: virtualRows,
  body: bodyRef,
  ...
  direction,
  overscan: () =>
    typeof props.virtualized === 'object' ? Number(props.virtualized.overscanCount ?? 2) : 2,
  estimate: () =>
    typeof props.virtualized === 'object' ? Number(props.virtualized.estimatedItemSize ?? 50) : 50,
  itemSize: virtualItemHeight,
  onScroll: (scrollOffset, scrollDirection, scrollUpdateWasRequested) => {
    if (typeof props.virtualized === 'object')
      props.virtualized.onScroll?.({ scrollOffset, scrollDirection, scrollUpdateWasRequested });
  },
});
const renderedRows = computed(() =>
  props.virtualized ? virtualList.window.value.rows : flatPageRows.value,
);
const virtualBodyStyle = computed(() => ({
  height: `${virtualList.window.value.height}px`,
  pointerEvents: virtualList.scrolling.value ? ('none' as const) : undefined,
  width: '100%',
}));
...
function columnQuery(column: NormalizedTableColumn<RecordType>): TableColumn<RecordType> {
  const source = getColumnSource(column);
  const query: TableColumn<RecordType> = Object.fromEntries(
    Object.entries(source).map(([key, value]) => [key, Array.isArray(value) ? [...value] : value]),
  );
  query.filteredValue = [...(filterValues.value.get(column.key) ?? [])];
  if (column.sorter) query.sortOrder = sortOrders.value.get(column.key) ?? false;
  return query;
}

function querySnapshot(filter?: {
  key: TableRowKey;
  values: unknown[];
}): Pick<TableChangeInfo<RecordType>, 'filters' | 'sorter'> {
  const columns = flattenColumns(declaredColumns.value);
  const queries = columns.map(columnQuery);
  const filters = queries
    .map((query, index) =>
      filter && columns[index]!.key === filter.key
        ? { ...query, filteredValue: [...filter.values] }
        : query,
    )
    .filter(
      (query, index) =>
        ...
        (query.filteredValue.length ||
          (columns[index]!.dataIndex && Array.isArray(columns[index]!.filteredValue))),
    );
  const sorter = queries.find((query) => query.sorter && query.sortOrder);
  return { filters, sorter };
}

function notifyChange(type: 'sorter' | 'filter' | 'pagination', query = querySnapshot()): void {
  emit('change', {
    extra: { changeType: type },
    ...query,
...
function handleSort(column: NormalizedTableColumn<RecordType>, event: Event): void {
  ...
  // The fixed adapter notifies from the previous query state while reporting
  // this click's requested sorter separately from the unchanged filter queries.
  const query = querySnapshot();
  const declaredColumn =
    flattenColumns(declaredColumns.value).find((item) => item.key === column.key) ?? column;
  query.sorter = { ...columnQuery(declaredColumn), sortOrder: next };
  if (column.sortOrder == null) {
    internalSortOrders.value = new Map(next ? [[column.key, next]] : []);
  }
  currentPage.value = 1;
  notifyChange('sorter', query);
  resetScroll();
  event.stopPropagation?.();
}

function handleFilter(column: NormalizedTableColumn<RecordType>, values: unknown[]): void {
  const query = querySnapshot({ key: column.key, values });
  if (!Array.isArray(column.filteredValue)) {
...
function measureTableWidth(): void {
  ...
  if (verticalScrollbarWidth.value === undefined) {
    // The pinned expanded row subtracts the platform scrollbar width even when
    // this particular body does not overflow vertically.
    ...
    verticalScrollbarWidth.value = probe.offsetWidth - probe.clientWidth;
    probe.remove();
  }
  const width = wrapperRef.value.getBoundingClientRect().width;
  if (measuredTableWidth.value === undefined || Math.abs(measuredTableWidth.value - width) >= 0.5)
    measuredTableWidth.value = width;
}

function measureHeaderWidths(): void {
  if ((!anyFixed.value && !props.virtualized) || !wrapperRef.value) return;
...
function handleBodyScroll(event: Event): void {
  ...
  if (props.virtualized) {
    if (previousVirtualNativeTop === target.scrollTop && typeof props.virtualized === 'object')
      props.virtualized.onScroll?.({ horizontalScrolling: true });
    previousVirtualNativeTop = target.scrollTop;
    virtualList.onNativeScroll(target);
  }
}
...
const bodyStyle = computed<StyleValue>(() =>
  props.virtualized
    ? {
        position: 'relative',
        height: virtualRows.value.length ? `${virtualViewport.value}px` : undefined,
        width: '100%',
        overflow: 'auto',
...
const fixedTableLayout = computed(() =>
  Boolean(
    props.scroll?.y ||
    props.sticky ||
    flattenColumns(declaredColumns.value).some((column) => column.fixed || column.ellipsis),
  ),
);
const tableStyle = computed<StyleValue>(() => ({
  width: toCssSize(props.scroll?.x),
  tableLayout: fixedTableLayout.value ? 'fixed' : undefined,
}));
...
function tableComponent(
  ...
  if (area === 'body' && props.virtualized) return 'div';
...
function ensureResizeObserver(): void {
  if (
    ...
    (anyFixed.value ||
      props.virtualized ||
      (showHeader.value && Boolean(props.scroll?.y || props.sticky)))
  ) {
...
watch(
  ...
  () => {
    // Column changes refresh measurements; scroll and resize refresh boundary classes.
    void nextTick(() => {
      if (resizeDisposed) return;
...
onMounted(() => {
  if (props.virtualized) props.getVirtualizedListRef?.({ current: virtualList.listRef });
  void nextTick(() => {
    if (resizeDisposed) return;
    if (typeof props.virtualized === 'object')
      props.virtualized.onScroll?.({
    ...
    <TableSpin :spinning="Boolean(props.loading)">
      <div :class="tableWrapClass">
          ...
          <template v-else>
            ...
            <span :class="`${prefix}-pagination-wrapper`">
              <Pagination
                ...
                v-bind="{ ...paginationConfig, onChange: handlePageChange }"
              />
            </span>
          </template>
        </div>
        ...
        <div :class="`${prefix}-container`">
          <div
            v-if="(props.scroll?.y || props.virtualized) && showHeader"
            ref="header"
          ...
          >
            <component
              ...
              :class="[prefix, fixedTableLayout ? `${prefix}-fixed` : undefined]"
              :style="tableStyle"
            >
              ...
              <TableHeader
                ...
                :header-widths="measuredHeaderWidths"
          ...
          <div ref="body" :class="`${prefix}-body`" :style="bodyStyle" @scroll="handleBodyScroll">
            <component
              ...
              :class="[prefix, fixedTableLayout ? `${prefix}-fixed` : undefined]"
              :style="props.virtualized ? { width: `${virtualWidth}px` } : tableStyle"
              :role="props.virtualized ? undefined : bodyRole"
              :aria-rowcount="props.virtualized ? undefined : pageData.length"
              :aria-colcount="props.virtualized ? undefined : displayColumns.length"
            >
              <component
                :is="bodyColgroupComponent('wrapper')"
                v-if="!props.virtualized"
                :class="`${prefix}-colgroup`"
              >
                <component
              ...
              <TableBody
                ...
                :expanded-inner-width="expandedInnerWidth"
                ...
                :virtualized="Boolean(props.virtualized)"
                :virtual-styles="props.virtualized ? virtualList.window.value.styles : undefined"
                :virtual-body-style="props.virtualized ? virtualBodyStyle : undefined"
                :virtual-column-widths="virtualColumnWidths"
                @expand="handleExpand"

...
export const virtualTreeSelectStyleId = 'virtual:workspace-tree-select-styles.css';
...
export const virtualNavigationStyleId = 'virtual:workspace-navigation-styles.css';
...
const resolvedVirtualTreeSelectStyleId = `\0${virtualTreeSelectStyleId}`;
...
const collapseStyleEntry = fileURLToPath(new URL('./src/collapse.scss', import.meta.url));
const codeHighlightStyleEntry = fileURLToPath(
...
const treeSelectStyleEntry = fileURLToPath(new URL('./src/tree-select.scss', import.meta.url));
...
const navigationStyleEntry = fileURLToPath(new URL('./src/navigation.scss', import.meta.url));
...
const styleEntries = new Map([
  ...
  [resolvedVirtualTreeSelectStyleId, treeSelectStyleEntry],
  ...
  [resolvedVirtualNavigationStyleId, navigationStyleEntry],
...
export function compilePinnedComponentStyles(): Plugin {
  return {
    ...
    resolveId(source) {
      ...
      if (source === virtualTreeSelectStyleId) return resolvedVirtualTreeSelectStyleId;
      ...
      if (source === virtualNavigationStyleId) return resolvedVirtualNavigationStyleId;

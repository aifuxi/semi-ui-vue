# AI 工作记录：UserGuide、DragMove 双语示例补齐

- 日期：2026-09-07
- 状态：补齐线完成；严格视觉验收未执行

按用户要求将上一轮三个独立组件拆分提交；当前提交累计纳入 UserGuide 8项、DragMove 4项。映射为 777/859，严格有效验收仍为43/859。
...
DragMove：Basic/CustomMove双语代表通过后扩展Constrainer/Handler。验证真实鼠标拖动、容器边界、仅图标启动拖动、right定位及点击变宽；中文小宽度60px、英文50px，均扩至100px，保留固定上游差异。Basic外层只预留140px空间，不限制拖动。

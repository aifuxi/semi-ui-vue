...
| 工作线             | 当前进度        | 剩余范围                                        | 下一批                                         |
| ------------------ | --------------- | ----------------------------------------------- | ---------------------------------------------- |
| 双语示例补齐       | 741/859 已映射  | 118 项待映射                                    | Feedback 7 项，然后 Notification 8 项          |
| 严格视觉与行为验收 | 43/859 有效验收 | 698 项已映射待验收，另有 118 项待补齐后进入验收 | Divider 2 项，然后 FloatButton 7 项、Grid 7 项 |
...
初始 163 项中，Icon 8 项、ConfigProvider 3 项、Locale 3 项、Dark Mode 2 项、Navigation 10 项已完成双语映射与严格验收；OverflowList 4 项、ScrollList 1 项和 Transfer 14 项已完成补齐线检查，严格验收待办仍保留，见 [OverflowList 记录](../../ai-work/20260907-103119-overflow-list-documentation-content.md)、[ScrollList 记录](../../ai-work/20260907-110551-scroll-list-documentation-content.md)与 [Transfer 记录](../../ai-work/20260907-113702-transfer-documentation-content.md)。以下保留原队列，跳过已补齐条目。从 Feedback 开始继续补齐，无需等待本批严格验收后再推进下一批。
...
## 验收线：698 项已映射待验收及后续入队示例

现有 698 项队列现在即可推进，不以补齐线完成为前提。按下表分类顺序、每类内部按 `coverage.json` 固定文档顺序选择尚未有效验收的示例；下一批为 Divider 2 项。同样按组件拆批，超过 20 项时按上游索引每 15 项拆分。展示类把动态浮层、图片、轮播和 Table 放在静态展示之后，输入类把选择器、日期时间、TreeSelect、Form、Upload 放在基础输入之后。

补齐线交付的新示例按所属分类加入验收队列，在下一次选批时纳入，不中断正在验收的批次；不属于下表分类的指南、工具、媒体与 AI 示例按补齐线表格顺序接续。下表列当前 698 项，包含新入队的 OverflowList 4 项、ScrollList 1 项和 Transfer 14 项，不是最终验收范围。选定批次仍需确认示例可运行，并完成章节/API/迁移审阅及完整严格矩阵；已有映射不代替这些检查。OverflowList 验收前需定位并修复已记录的 collapse/scroll 计数更新问题；ScrollList 须统一双侧分钟禁用数据；Transfer 须审阅原生拖放、删除按钮和自定义面板空态适配差异。

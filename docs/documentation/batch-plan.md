...
| 严格视觉与行为验收 | 45/859 有效验收 | 814 项已映射待验收 | FloatButton 7 项，然后 Grid 7 项 |
...
Button 17 项试点已建立独立、可追溯且随源码变化失效的验收流程。现有七批有效证据由 2026-09-11 的 240 项正式矩阵确认；只有正式验收成功才能增加 `accepted`。
...
现有 814 项队列现在即可推进，不以补齐线完成为前提。按下表分类顺序、每类内部按 `coverage.json` 固定文档顺序选择尚未有效验收的示例；Divider 2 项已完成，下一批为 FloatButton 7 项。同样按组件拆批，超过 20 项时按上游索引每 15 项拆分。展示类把动态浮层、图片、轮播和 Table 放在静态展示之后，输入类把选择器、日期时间、TreeSelect、Form、Upload 放在基础输入之后。

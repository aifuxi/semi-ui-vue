...
| 严格视觉与行为验收 | 346/859 当前有效 | 0 项待恢复；513 项未验收 | Table 第 31–37 项；29 待办 |
...
2026-09-13 正式验收工具增加前置审阅门禁与失败即停，当时 44 批旧报告因工具输入变化待重验，历史累计通过仍为332项，尚未验收527项。新门禁发现 Divider、Notification、Popconfirm、Toast 缺少章节/API/迁移审阅，见[改进记录](../../ai-work/20260913-074247-documentation-acceptance-guardrails.md)。先由三个子 agent 准备并交叉审阅这四批，补齐真实语义审阅及正文 API 边界，73 项代表诊断与 184 项完整历史矩阵全部通过，恢复 23 个有效示例，详见[四批恢复记录](../../ai-work/20260913-4-batch-historical-review.md)。随后完成其余 40 批、309 个历史示例的恢复：936 项代表诊断及 2348 项完整矩阵分 13 组全部通过，无失败、跳过或重试，未修改共享源码或旧审阅指纹。当前 44 批审阅及浏览器证据全部有效，历史待恢复清零；首次新增验收 0 项，剩余 527 项待首次验收，下一批 Table 第 16–30 项，详见[309 项恢复记录](../../ai-work/20260913-105402-historical-309-acceptance.md)。下文历次通过数字保留当时事实。

2026-09-13 接续本轮由三个子 agent 并行准备 Table 第 16–28、30 项，新增 14 项；第 29 项 DragSort 的真实第三方交互差异保留待办。43 项代表及 112 项完整新增矩阵通过，Table 分组、树展开与列宽契约修复通过源码、产物和组件浏览器检查；真实参考 resize 依赖使历史 44 批失效后，2532 项完整历史矩阵分 16 组全部恢复。最终 45 批审阅及浏览器证据有效，346/859 当前有效，剩余 513 项待首次验收，下一批 Table 第 31–37 项并保留第 29 项待办，详见[本轮工作记录](../../ai-work/20260913-123247-table-2-documentation-acceptance.md)。
...
## 验收线：513 项已映射待验收及后续入队示例

现有 513 项队列现在即可推进，不以补齐线完成为前提。按下表分类顺序、每类内部按 `coverage.json` 固定文档顺序选择尚未有效验收的示例；Divider 2 项、FloatButton 7 项、Grid 7 项、Layout 8 项、Resizable 14 项、Space 5 项、Typography 10 项、Accessibility 1 项、Banner 4 项、Feedback 7 项、Notification 8 项、Toast 9 项、Popconfirm 4 项、Progress 12 项、Skeleton 8 项、Spin 5 项与展示类静态组件 Avatar 15 项、Badge 6 项、Calendar 9 项、Card 14 项、Collapse 6 项、Collapsible 4 项、Descriptions 8 项、Empty 5 项、Highlight 4 项、List 11 项、OverflowList 4 项、ScrollList 1 项、Tag 12 项、Timeline 8 项、Carousel 8 项、Cropper 5 项、Dropdown 6 项、Image 10 项、Modal 12 项、Popover 9 项、SideSheet 6 项与 Table 第 1–28、30 项已完成（ScrollLoad、Virtualized、DragSort 因第三方依赖记录后排除，仍留在队列中），下一批为展示类 Table 第 31–37 项，第 29 项 DragSort 仍保留待办。同样按组件拆批，超过 20 项时按上游索引每 15 项拆分。展示类把动态浮层、图片、轮播和 Table 放在静态展示之后，输入类把选择器、日期时间、TreeSelect、Form、Upload 放在基础输入之后。

补齐线交付的新示例按所属分类加入验收队列，在下一次选批时纳入，不中断正在验收的批次；不属于下表分类的指南、工具、媒体与 AI 示例按补齐线表格顺序接续。下表列当前 513 项，包含新入队的 Transfer 14 项、UserGuide 8 项、DragMove 4 项和 HotKeys 5 项，不是最终验收范围。选定批次仍需确认示例可运行，并完成章节/API/迁移审阅及完整严格矩阵；已有映射不代替这些检查。Transfer 须审阅原生拖放、删除按钮和自定义面板空态适配差异；UserGuide 须对照本地封面替换、完整位置/高亮/受控/动效矩阵；DragMove 须对照双语小宽度及自定义 right 定位；HotKeys 须对照默认快捷键动作和局部监听边界。

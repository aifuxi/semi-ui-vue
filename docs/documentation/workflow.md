...
当前 722/859 是已有映射数量，不据此追认全部示例已通过统一运行检查。基础加载、编译和运行错误属于补齐线必须修复的问题；已识别但不阻断演示的精细对齐差异记录到现有批次材料或工作记录，交给验收线处理，不算 accepted deviation。最终完成标准不变。

## 补齐线流程

1. 按补齐队列确定固定上游索引，检查 Git 状态、vendor 基线、双语路径精确大小写和多文件依赖；对照本地上游补齐内容与映射，注明中英文顺序或独有示例差异。
2. 执行文档联合检查，启动实际站点，逐项检查本批中英文示例的加载、运行和主要演示操作；涉及编辑器或多文件依赖时验证对应运行路径。修复阻断运行的问题，不把基础检查扩大成严格 React/Vue 对照。
3. 刷新覆盖账本，记录本批运行证据、尚待严格验收的差异及下一补齐批次。仅补齐示例时不要求创建严格验收 spec 或执行正式矩阵；完成本批后可继续下一批。
4. 若改动组件或共享设施，按影响范围执行必要回归并检查历史证据失效范围；失效批次进入验收线重验，不能为维持计数修改指纹或沿用旧证据。分别报告映射进度与有效严格验收进度。

...
[coverage.json](./coverage.json) 记录 102 组固定上游文档及每个中文 live Demo 的章节和源码行号。当前 859 个中文上游 Demo 中，859 个已建立双语内容与示例映射，有效严格验收为 141 个（Button 17 项、Icon 8 项、ConfigProvider 3 项、Locale 3 项、Dark Mode 2 项、Navigation 10 项、Divider 2 项、FloatButton 7 项、Grid 7 项、Layout 8 项、Resizable 14 项、Space 5 项、Typography 10 项、Accessibility 1 项、Banner 4 项、Feedback 7 项、Notification 8 项、Toast 9 项、Popconfirm 4 项、Progress 12 项，二十批共通过 1008 条双语/明暗/适用 RTL 用例）。Icon 短 fill 调色板顺序缺陷已修复，并通过全仓 Chromium 与真实发布包回归。此前补齐 Form（39）、Table（37）、Upload（42）、TreeSelect（19）、Tree（27），共新增 164 个上游 Demo 的双语映射。中英文上游章节数量与顺序不同的条目已在各组件 mapping 中注明；映射数量不代表逐项视觉验收完成。总数 859 来自固定源码的 live Demo 解析，修正了旧统计遗漏的 4 个示例。
...
| 严格视觉与行为验收 | 141/859 有效验收，718 项已映射待验收 | Skeleton 8 项            |
...
2026-09-10 已恢复历史六批的当前有效证据：224/224 正式矩阵通过，无重试、跳过或失败，覆盖计数为 43/859。修正了分页更新、语言切换后的省略测量及子菜单退出动画的采样时序，组件运行时和视觉阈值未变。详见 [恢复验收记录](../../ai-work/20260910-002700-restore-six-documentation-batches.md)。

2026-09-11 完成 Divider 严格验收并恢复失效的历史六批：240/240 正式矩阵通过，无重试、跳过或失败，覆盖计数为 45/859。修复 React 参考服务按需编译引发的跨页面重载；独立稳定性回归与 442 项组件 Chromium 对照通过。见 [本轮记录](../../ai-work/20260911-142100-divider-release-readiness.md)与 [Divider 审阅矩阵](divider-acceptance.md)。
...
2026-09-11 完成 Toast 九项严格验收（含英文独有 Stacking）：72/72 双语、明暗、LTR/RTL 正式矩阵通过；hook holder 结构修复使 ConfigProvider、Resizable、Typography 证据失效，四批共享一次构建重验，无重试、跳过或失败，覆盖计数为 125/859。修复 `useToast` 复用命令式 host 导致空 holder 常驻 innerWrapper、hook 弹层多一层 fit-content 祖先的问题，并让 `positionInList` 缺省时位移为 0；验证 3s/10s 边界、同 id 更新重启、leading-only 节流、堆叠 hover 展开与 3D 缩放、自定义容器、进出动效与真实编辑运行。见 [本轮记录](../../ai-work/20260911-234654-toast-documentation-acceptance.md)与 [Toast 审阅矩阵](toast-acceptance.md)。

2026-09-12 完成 Popconfirm 四项严格验收：32/32 双语、明暗、LTR/RTL 正式矩阵通过，无重试、跳过或失败，覆盖计数为 129/859。修复 `RadioGroup` 丢弃 slot 顶层 `v-for`（`Fragment`）导致模板内 `Radio` 全部不渲染的缺陷；`Types` 示例去掉固定上游没有的局部容器与 `getPopupContainer`，浮层回到 body 级 Portal；站点 `:focus-visible` 焦点环不再套到 Portal 内的示例内容上。三个改动分别落在组件源码与全批次共享的站点样式上，19 批历史证据一并失效，按 `--affected` 的完整集合重验 19 批 912 项（因单次报告超过 V8 字符串上限分 5 组共享同一份准备证明运行），442 项全仓组件 Chromium 对照回归与产物门禁通过。见 [本轮记录](../../ai-work/20260912-021340-popconfirm-documentation-acceptance.md)与 [Popconfirm 验收记录](popconfirm-acceptance.md)。

2026-09-12 完成 Progress 十二项严格验收：96/96 双语、明暗、LTR/RTL 正式矩阵通过，无失败、重试或跳过；章节/API/迁移审阅与本批门禁有效。对齐双语示例的容器、自定义文本和英文固定颜色/可访问名称，修正文档 Vue 引入和迁移表绑定语法。覆盖总数为 141/859，原十九批证据保持有效，下一批 Skeleton 8 项。见 [本轮记录](../../ai-work/20260912-030413-progress-documentation-acceptance.md)与 [Progress 验收记录](progress-acceptance.md)。

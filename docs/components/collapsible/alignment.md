...
- 根与 `@aifuxi/semi-ui-vue/collapsible` 子路径 default/named 导出组件和公开类型；`@aifuxi/semi-theme-default/collapsible.css` 包含固定过渡样式。真实 tarball 验证 ESM、声明、样式入口、tree-shaking、SSR-safe import、许可证和 SPDX SBOM。

...
React/Storybook 默认端口为 4173/4174，手动调试分别使用 `pnpm dev:reference`、`pnpm dev`；Storybook 单独构建使用 `pnpm build:storybook`。组件库文档站使用端口 4321，首次运行前先执行一次 `pnpm docs:prepare` 构建公开包与主题包，之后使用 `pnpm docs:dev`、`pnpm docs:build`，站点生成物只写入 `apps/docs` 的已忽略目录。正式组件测试不复用已有服务，启动前先处理相同端口占用。并行工作共享代码和构建产物，由一个执行者管理服务。仓库内 worktree 放在已忽略的 `.worktrees/`。

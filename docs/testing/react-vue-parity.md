...
预构建的来源证据由 `parity-provenance.json` 记录实际代码块模块及被优化掉的静态转导出入口。测试只认可当前页面实际请求的代码块，不把未请求的动态代码块算作来源；映射缺失必须失败。开发模式同样从 Rspack 实际模块图生成映射，并在读取时获取最新版本，避免 HMR 后使用旧证据。`pnpm benchmark:parity dev`（也支持 `warm`、`build`）是单独的加载实验，不替代本节的完整门禁，其生产构建实验与正式预构建开发模式的耗时不可直接互换，报告区分准备、首次加载和重复加载耗时。

两个应用使用 Rsbuild 2.2.5，保留独立类型检查。React 工作台同时输出 `index.html` 和 `docs.html`；文档适配器使用独立求值实例加载整个依赖图，并将递归 helper 和适配器目录接入 Rspack watch，支持更新以及批次增删。React 16 参考源码明确使用 classic JSX 编译，保留既有子节点语义。
...
`apps/reference-react/rsbuild.config.ts` 将 `@semi-v2.102.0/button` 精确解析到：

```text
vendor/semi-design/packages/semi-ui/button/index.tsx
```

Button 的运行依赖使用固定版本，并通过 alias 保证从参考应用解析。样式由 Sass 1.54.9 将主题 Token、global、Button、IconButton 和 icons SCSS 编译为虚拟 CSS 模块；上游 TSX 的对应副作用样式导入仅做去重。这样既不修改 vendor，通过共享解析插件接入 Rspack 的 CSS 流程。

浏览器测试监听真实模块请求，开发和预构建模式都核对已请求代码块的来源映射，要求包含上述 vendor 公开入口；同时独立断言 `.semi-button-*` 公开 class、32px 默认高度、3px 圆角、字体、内边距与点击行为。
...
Vue 对照场景必须从 `@aifuxi/semi-ui-vue/<component>` 公开子路径导入。场景注册仍使用 `import.meta.glob` 按需加载，但禁止场景回到组件包根入口，避免根入口的全部 re-export 扩大场景依赖图。工作台以 Divider 场景不超过 200 个浏览器请求作为回归门禁，并同时断言请求来源包含 `divider/index.ts`、不包含组件包根 `index.ts`；两种服务模式都保留此检查。
